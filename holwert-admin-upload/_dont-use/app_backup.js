class HolwertAdmin {
            ...
            <div class="org-content-list">
                ${events.map(event => `
                    <div class="content-item">
                        ...
                        <div class="content-actions">
                            <button class="btn btn-sm btn-secondary" onclick="admin.openEventModal(${event.id})">
    ...
    // Uniforme Event Modal (voor zowel nieuw als bewerken)
    async openEventModal(eventId) {
        try {
            ...
            if (eventId) {
                mode = 'edit';
                // Haal event data op voor bewerken
                ...
                <div class="modal">
                    ...
                    <div class="modal-body">
                        <form id="eventForm" class="form">
                            <div class="form-group">
                                <label>Titel</label>
                                <input type="text" id="evTitle" value="${initial.title}" placeholder="Titel" required>
                            </div>
                            <div class="form-group">
                                <label>Omschrijving</label>
                                <textarea id="evDesc" style="min-height:120px" placeholder="Omschrijving" required>${initial.description}</textarea>
                            </div>
                            <div class="form-row">
                                <div class="form-group">
                                    <label>Begindatum</label>
                                    <input type="datetime-local" id="evStart" value="${initial.event_date}" required>
                                </div>
                                <div class="form-group">
                                    <label>Einddatum</label>
                                    <input type="datetime-local" id="evEnd" value="${initial.event_end_date}" required>
                                </div>
                            </div>
                            <div class="form-group">
                                <label>Locatie</label>
                                <input type="text" id="evLocation" value="${initial.location}" placeholder="Locatie" required>
                            </div>
                            <div class="form-group">
            ...
            // Koppel einddatum aan begindatum
    ...
    async saveEvent(eventId) {
        try {
            ...
            if (!title || !description || !event_date_raw || !location) {
                this.showNotification('Titel, omschrijving, begindatum en locatie zijn verplicht', 'error');
                return;
            }
